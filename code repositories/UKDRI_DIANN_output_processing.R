...
colnames(Aonly) <- gsub(".*DRIX","DRIX",colnames(Aonly))
...
m.Aonly <- data.matrix(Aonly[,-c(1:4)])
...
colnames(m.Aonly) <- colnames(Aonly)[-c(1:4)]
...
cvc.vol <- CasevsControl1

cvc.vol <- cvc.vol[complete.cases(cvc.vol),]

cvc.vol$name <- proteinid$Name

cvc.vol$label <- NA
cvc.vol$label[cvc.vol$P.Value <0.05 & cvc.vol$logFC > 1] <- cvc.vol$name[cvc.vol$P.Value <0.05 & cvc.vol$logFC > 1]
cvc.vol$label[cvc.vol$P.Value <0.05 & cvc.vol$logFC < -1] <- cvc.vol$name[cvc.vol$P.Value <0.05 & cvc.vol$logFC < -1]

cvc.vol$col <- "neither"
cvc.vol$col[cvc.vol$P.Value <0.05 & cvc.vol$logFC > 1] <- "UP"
cvc.vol$col[cvc.vol$P.Value <0.05 & cvc.vol$logFC < -1] <- "DOWN"


cvc.vol$P.Value <- -log10(cvc.vol$P.Value)


p <- ggplot(cvc.vol,aes(logFC,P.Value, label = cvc.vol$label, col = cvc.vol$col))+
  geom_point(show.legend = F)+
  geom_hline(yintercept = 1.301, col = "darkblue", lty = "dashed")+
  geom_hline(yintercept = 0, col = "black", alpha = 0.6)+
  geom_vline(xintercept = 0, col = "black", alpha = 0.6)+
  geom_vline(xintercept = 1, col = "darkred", lty = "dashed")+
  geom_vline(xintercept = -1, col = "darkred", lty = "dashed")+
  geom_label_repel(size =3 ,alpha = 1,show.legend = F,max.overlaps = 100)+
